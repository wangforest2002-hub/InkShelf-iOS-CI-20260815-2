...
- 夜间模式使用夜灯和月光氛围，保证文字、进度和按钮仍有足够对比度，同时继续显示完整书架。
- 默认动效包含窗外云朵漂移、灯光呼吸、暖光掠过、内容轻柔入场和卡片按压反馈；开启系统“减少动态效果”后自动收敛。
...
- 独立“画廊”把外部导入图片、阅读中收藏的单页和收藏的整本读物分开陈列。
- 书架与画廊可一键切换日间/夜间模式；夜间模式不再建立另一套书架。
...
面向已经成年的个人用户，应用可以为用户自行导入的漫画与画集保存成年向心动档案：

- 给每本作品保存心动指数、涩气指数、标签与私人读后感，形成只属于自己的偏好画像。
- 按甜欲、暧昧、姐姐气场、亲密、大胆与作画等氛围标记作品；书卡会显示对应成年向标识。
- DeepSeek 保留“坏心眼姐姐”“大胆同好”陪读性格和“夜读私语”写作类型。
- 成年向特色只用于明确为成年角色的作品，不收录或性化未成年及年龄不明角色。

2.3.0 起取消独立“夜读”书架，改为全局日间/夜间模式切换。夜间模式与普通模式拥有相同的书架、分组、搜索、最近阅读、画廊和打开能力，成年向标签与心动档案继续保留；当前测试阶段不加入隐私锁或封面遮罩。

## 流畅阅读

2.5.3 统一书架、画廊、阅读控制层与设置的视觉细节：iPad 首页的信息卡在宽屏并排，画廊复用书架的横竖封面混排，卡片按压和指针反馈保持轻盈。缩略图使用独立的小图缓存，PDF 缩略图通过独立文档在后台串行生成；拖动阅读进度期间工具栏保持可见。环境动效在低电量模式、严重发热、后台或不可见页面自动暂停，系统“减少动态效果”继续生效。版本更新保留现有应用身份与本地数据格式。
...
- 2.3.0 将主导航改为“书架 / 最近 / 画廊 / 设置”；“画廊”独立区分外部导入图片、阅读收藏单页与整本收藏，并把原独立夜读页改为全局夜间模式。
- 2.3.1 取消图片阅读器中过期的异步定位请求，并统一 PDF 双页回调的跨页锚点；快速翻页、拖动进度和切换单双页后不会再被旧页码拉回。
- 2.4.3 将漫画与画集的分页阅读迁移到 iOS 系统级页面容器，每次手势只提供相邻页，从底层消除长画册前半段单数页跳跃；进度条改为松手后精确定位，拖动期间不再反复触发高清页加载。
- 2.4.3 流畅度重构把书架封面、收藏单页、画册索引和 PDF 建档移出触摸线程；阅读期间只预热相邻页，书架进度与成就写入后台串行保存，不再让隐藏书架跟着每次翻页刷新。
- 2.4.3 的暖光、窗口、夜读呼吸和扫光仍完整保留，但不可见标签页、弹窗背后及阅读器背后的环境动画会自动休眠；动态节奏统一为按压、数值、工具栏、面板与入场五类，避免多层弹簧互相打架。
- 2.4.2 完整恢复 2.4.0 的阅读与翻页实现，仅提高版本号用于覆盖回退 2.4.1；书架、缓存、收藏和阅读进度继续沿用原应用数据容器。
- 2.4.0 新增可切换的纸张翻页动效，横向画集带有跟手透视、书脊阴影和页缘高光；统一书架、画廊、弹层与阅读工具栏的动效节奏，并自动适配系统“减少动态效果”。导入时会按实际内容提醒完全重复的文件，设置中也可扫描旧书架、对照并删除多余副本。
...
```powershell
.\scripts\Publish-OnlineUpdate.ps1 -SignedIpa "D:\path\二次元小家-signed.ipa" -Version "2.4.3" -Build 27 -Notes "修复长画册前半段跳页","封面与 PDF 打开不再阻塞界面","隐藏动画自动休眠，阅读与书架更流畅","覆盖安装保留书架与缓存"
```

## X 图片珍藏与 Sharp 清晰化

- 在书架分组内发起导入，新读物会直接进入当前分组；从“画廊”导入的个人图片进入“我的图片”，不会自动混入阅读中收藏的心动单页。
