...
2.5.3 统一书架、画廊、阅读控制层与设置的视觉细节：iPad 首页的信息卡在宽屏并排，画廊复用书架的横竖封面混排，卡片按压和指针反馈保持轻盈。缩略图使用独立的小图缓存，PDF 缩略图通过独立文档在后台串行生成；拖动阅读进度期间工具栏保持可见。环境动效在低电量、严重发热、后台或不可见页面自动暂停，系统“减少动态效果”继续生效。版本更新保留现有应用身份与本地数据格式。
